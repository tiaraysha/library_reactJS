...
export default function Restoration() {
  ...
  const filteredLendings = selectedMonth
    ? lendings.filter((item) => {
        const month = new Date(item.tgl_pinjam).getMonth() + 1;
  ...
  useEffect(() => {
    ...
    setFormDenda((prev) => ({
      ...prev,
      jumlah_denda: jumlah === "" ? "" : String(jumlah),
    }));
  ...
  function handleBtnCreate(lending) {
    ...
    setFormDenda({
      ...
      jumlah_denda: "",
      deskripsi: "",
      tgl_pinjam: lending.tgl_pinjam, // pastikan field ini diisi
      tgl_pengembalian: lending.tgl_pengembalian, // pastikan field ini diisi
    });
    ...
    const returnOnly = new Date(
      returnDate.getFullYear(),
      returnDate.getMonth(),
      returnDate.getDate()
    );

    if (lending.status_pengembalian == 0 && today > returnOnly) {
      const diffTime = todayOnly - returnOnly;
  ...
  function handleSubmitForm(e) {
    ...
    axios
      .put(
        API_URL + `/peminjaman/pengembalian/${detailLending.id}`,
        formModal,
        {
          headers: {
            Accept: "application/json",
  ...
  function handleBtnDenda(e) {
    ...
    // Pastikan jumlah_denda string
    const dendaToSend = {
      ...formDenda,
      jumlah_denda:
        formDenda.jumlah_denda === undefined
          ? ""
          : String(formDenda.jumlah_denda),
    };

    console.log("Form Denda yang dikirim:", dendaToSend);

    axios
      .post(API_URL + "/denda", dendaToSend, {
        headers: {
          Accept: "application/json",
      ...
      .then((res) => {
        console.log("Respon sukses denda:", res.data);
        return axios.put(
          API_URL + `/peminjaman/pengembalian/${detailLending.id}`,
          formModal,
          {
      ...
      .then((res) => {
        console.log("Respon sukses pengembalian:", res.data);
        setIsModalOpen(false);
      ...
      .catch((err) => {
        console.log("Error pada proses denda/pengembalian:", err, err.response);
        if (err.status == 401) {
          localStorage.removeItem("token");
          navigate("/");
        }
        setError(
          err.response?.data || {
            message: "Terjadi kesalahan saat proses denda dan pengembalian.",
          }
        );
      });
  }
  ...
  function exportExcel() {
    ...
    const formattedData = filteredLendings.map((item, index) => ({
      No: index + 1,
      IdMember: item.id_member,
      IdBuku: item.id_buku,
      TanggalPinjam: new Date(item.tgl_pinjam).toLocaleDateString("id-ID", {
        dateStyle: "long",
      }),
      TanggalPengembalian: new Date(item.tgl_pengembalian).toLocaleDateString(
        "id-ID",
        { dateStyle: "long" }
      ),
      StatusPengembalian:
    ...
    <>
      ...
      >
        <h3 className="text-start mt-3">
          Daftar Pengembalian <FontAwesomeIcon icon={faMoneyBillTransfer} />
        </h3>
  ...
  <div className="flex items-center gap-2">
    ...
    <select
      className="appearance-none border-b border-gray-400 bg-transparent text-gray-700 text-sm px-1 py-0 focus:outline-none focus:border-amber-500"
      value={selectedMonth}
    ...
    >
      ...
      {Array.from({ length: 12 }, (_, i) => (
        <option key={i + 1} value={i + 1}>
          ...
          <tbody>
            {filteredLendings.map((value, index) => {
              return (
                <tr key={value.id}>
                  ...
                  <td>{value.id_buku}</td>
                  <td>
                    {new Date(value.tgl_pinjam).toLocaleDateString("id-ID", {
                      dateStyle: "long",
                    })}
                  </td>
                  <td>
                    {new Date(value.tgl_pengembalian).toLocaleDateString(
                      "id-ID",
                      { dateStyle: "long" }
                    )}
                  </td>
                  <td>
                    {value.status_pengembalian == 0 ? (
                      <p className="text-danger fw-bold">Belum dikembalikan</p>
                    ) : (
                      <p className="text-success fw-bold">Sudah dikembalikan</p>
                    )}
                  </td>
                  <td className="w-25 text-center">
                    {value.status_pengembalian == 1 ? (
                      <button
                        className="btn btn-outline-info"
                        onClick={() => handleBtnDetail(value)}
                      >
                        Detail Pengembalian
                      </button>
                    ) : (
      ...
      >
        ...
        <form onSubmit={handleSubmitForm}>
          <div className="form-group">
            <p>
              Apakah buku dengan <b>id: {detailLending.id_buku}</b> sudah
              dikembalikan?
            </p>
          </div>
        ...
        <form onSubmit={handleBtnDenda}>
          ...
          <div className="form-group">
            <label className="form-label fw-bold">
              id_member <span className="text-danger">*</span>
            </label>
            <input
              type="text"
            ...
            <label className="form-label fw-bold">
              id_buku <span className="text-danger">*</span>
            </label>
            <input
              type="text"
              className="form-control"
            ...
            <label className="form-label fw-bold">
              Jenis Denda <span className="text-danger">*</span>
            </label>
            <select
            ...
            <label className="form-label fw-bold">
              jumlah_denda <span className="text-danger">*</span>
            </label>
            <input
              type="text"
              className="form-control"
      ...
      {/* DETAIL RESTORATION */}
      <Modal
        isOpen={isModalRestorationOpen}
        onClose={() => setIsModalRestorationOpen(false)}
      ...
      >
        <div className="form-group mt-2">
          <ol>
            <li>
              Tanggal Peminjaman:{" "}
              <b>
                {new Date(detailRestoration.tgl_pinjam).toLocaleDateString(
                  "id-ID",
                  { dateStyle: "long" }
                )}
              </b>
            ...
            <li>
              ...
              <b>
                {new Date(
                  detailRestoration.tgl_pengembalian
                ).toLocaleDateString("id-ID", { dateStyle: "long" })}
              </b>
            </li>
            <li>
              Id_buku: <b>{detailRestoration.id_buku}</b>
            </li>
            <li>
              Id_member: <b>{detailRestoration.id_member}</b>
            </li>
          </ol>
        </div>
        <div className="modal-footer">
          <button
            type="button"
            className="btn btn-secondary"
            onClick={() => setIsModalRestorationOpen(false)}
          >
            Close
          </button>
        </div>
      ...
      >
        <div className="d-flex justify-content-end">
          <button className="btn btn-danger" onClick={exportPdf}>
            Export <FontAwesomeIcon icon={faFilePdf} />
          </button>
        </div>
        <div className="form-group mt-2">
          {member.map((item, index) => {
            return (
              <div key={index} className=" m-3 p-3 border rounded shadow">
                <p>
                  <strong>ID Peminjaman:</strong> {item.id}
                </p>
                <p>
                  <strong>ID Buku:</strong> {item.id_buku}
                </p>
                <p>
                  <strong>Tanggal Pinjam:</strong> {item.tgl_pinjam}
                </p>
                <p>
                  <strong>Tanggal Pengembalian:</strong> {item.tgl_pengembalian}
                </p>
                <p>
